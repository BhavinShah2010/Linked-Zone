...
<!--
 ...
 <?php    
	...
	$file_path = "admin/all_admin_img/user_".$user_id."/".$filename;
	
	$obj = PHPExcel_IOFactory::load($file_path);
	echo "<hr>";
	$data = $obj->getActiveSheet ()->getRowIterator ();//toArray (null , true , true , true);
	$i = 0 ; $j = 0;
	//echo '<table border="1" style="border:thin solid #CCC; id="View_Report"">';
	echo '<table border="1" id="View_Report"">';
	echo "<thead>";
...
//		 echo $row->getRowIndex();
		 echo '<tr align="justify" style="color:#82ab00; margin-top:1%;"  >';
		 $j = 0;
		 foreach ($cellIterator as $cell)
		  {
			  if ($i == 0)
			  {
			   		echo "<th>".$cell->getCalculatedValue ()."</th>";
			  }
			   else
			   {
