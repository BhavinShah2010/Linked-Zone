	...
	if (isset($_SESSION['user_id']))
 	{
		...
		$query = "SELECT pattern_id, first_name , last_name FROM user_reg_tb ORDER BY first_name ASC ";	
		$result = mysqli_query($con,$query);
...
<link rel="stylesheet" href="styles/block_user.css" />
...
<tbody>
    ...
    $sql = mysqli_query($con,"select * from user_reg_tb WHERE user_del_status = 0") or die("error");
	//$sql1 = mysqli_query($con,"select * from  ");
    while($data = mysqli_fetch_array($sql))
    {	
		 if ($data['user_block_status'] == 1)
		 {
		echo '<tr id="block_id_'.$data['pattern_id'].'" style="background:#ccc;">';
        echo '<td align="justify" style="background:#ccc" >';
		echo '	<input type="checkbox" id="check" name ="user" value ="'.$data['pattern_id'].'">';
		echo '</td>';
                       
        echo '<td align="justify">';
		echo '	<a style="text-decoration:none;" href="../profile.php?profile_user="'.$data['pattern_id']. '>'.$data['first_name'].' '.$data['last_name'];
		echo '</a></td>';
		echo '<td align="justify">'.$data['user_id'].'</td>';
		//echo '<td>'.$data['block_status'].' </td>';
		//echo '<td><img src="all_user_img\user_'.$data['pattern_id'].'\profile_pic.jpg" height=25 width=25/></td>';
		echo '<td><img src="all_user_img/user_1/profile_pc.jpg"></td>';
        ...
        echo '</tr>';
		 ...
		 {
         echo '<tr id="block_id_'.$data['pattern_id'].'">';
        echo '<td align="justify">';
		echo '	<input type="checkbox" id="check" name ="user" value ="'.$data['pattern_id'].'">';
		echo '</td>';
...
<!-- Akhtra Div for getting Hr-->
            ...
            <div id="block_box">
            	<div id="block_btn_box">
                    <input type="button" value="Un Block User" class="unblock_btn"/>
                    <input type="button" value="Block User" class="block_btn"/>
                </div>
