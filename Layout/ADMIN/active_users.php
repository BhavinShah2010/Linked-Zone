...
<link rel="stylesheet" href="styles/total_users.css" />
...
<div id="main_box" ></div>
	...
	<div id="container" data-role="none">
        ...
        <div id="title">
           ...
           <!-- <div id="main_cont">
        ?php
			$total_users = mysqli_query($con,"select * from user_reg_tb") or die(mysqli_error());
		 	//$q2 = mysqli_query($con,"select * from user_reg_tb where login_status=1");
			
		 	//$q3 = mysqli_query($con,"select * from user_reg_tb where login_status=0");
			
			while($row=mysqli_fetch_array($total_users))
			{
			  
				 echo  '<div class="users_div">';
				 echo  '<div class="users_pic">';  
				 echo '<img src="all_user_img\user_'.$row['pattern_id'].'\profile_pic.jpg" width="80" height="80"/></img>';
                  	
                 echo  '</div>';
 		         echo  '<div class="name_box">';
                 //echo  '<span>'. $row["first_name"].$row["last_name"] .'</span>';
				 echo '<span><a style="color:#82ab00;text-decoration:none;" href="../profile.php?profile_user="'.$row['pattern_id']. '>'.$row['first_name'].' '.$row['last_name'].'</a><span>';
                 echo  '</div>';
				 echo  '</div>';
		...
		?>
          <!--  	<div class="users_div">
                        ...
                        <div class="name_box">
                        	<span> surjeet singh bhaduriya</span>
                        </div>
				</div>-- 
...
<tbody>
    ...
    $sql = mysqli_query($con,"SELECT * from user_reg_tb WHERE login_status = 1") or die("error");
    ...
    while($data = mysqli_fetch_array($sql))
    {
       ...
       // echo '<td align="justify" ><input type="checkbox" id="check" name ="user" value ="'.$data['pattern_id'].'"</td>';
		...
		echo '<td><img src="all_user_img\user_'.$data['pattern_id'].'\profile_pic.jpg" height=40 width=40/></img></td>';
